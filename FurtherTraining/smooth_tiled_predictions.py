...
def _pad_img(img, window_size, subdivisions):
    ...
    # Ensure the pad_width is 2D for 3D images (e.g., color images)
    pad_width = [(aug, aug), (aug, aug)]
    
    # Check the number of dimensions in the image
    if len(img.shape) == 3:
        # If it's a color image, add a pad width for the third dimension
        pad_width.append((0, 0))
    
    ret = np.pad(img, pad_width=pad_width, mode='reflect')
